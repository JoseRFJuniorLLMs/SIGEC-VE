...
class ChargePoint(OCPPCp): # Agora ChargePoint herda de OCPPCp (o ChargePoint da biblioteca)
    @on('BootNotification')
    async def on_boot_notification(self, **kwargs):
        ...
        logger.info(f"CP {self.id}: StatusNotification para conectores enviado após BootNotification.")
    ...
    async def on_remote_start_transaction(self, connector_id: int, id_tag: str, **kwargs):
        logger.info(
    ...
    async def on_remote_stop_transaction(self, transaction_id: int, **kwargs):
        logger.info(f"CP {self.id}: Recebida RemoteStopTransaction para transação {transaction_id}")
        # Simula a aceitação do comando
    ...
    async def on_change_availability(self, connector_id: int, type: AvailabilityType, **kwargs):
        logger.info(f"CP {self.id}: Recebida ChangeAvailability para conector {connector_id} tipo: {type}")
...
async def start_charge_point(cp_id: str, csms_url: str):
    ...
    try:
        async with websockets.connect(csms_url + f"/{cp_id}", subprotocols=['ocpp1.6']) as websocket:
            ...
            await charge_point.send_boot_notification(
                charge_point_model="SIGEC-CP-Sim",
                charge_point_vendor="SIGEC"
            )
